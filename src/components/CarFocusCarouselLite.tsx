...
const CarFocusCarouselInner = ({ initialCarId, onCarChange }: CarFocusCarouselProps, ref: React.Ref<CarFocusCarouselHandle>) => {
  ...
  const FlipInner = ({ car, flipped }: { car: any; flipped: boolean }) => {
    ...
    const faceStyle: React.CSSProperties = { position: 'absolute', inset: 0, backfaceVisibility: 'hidden', transform: 'translateZ(0)' };
    const backStyle: React.CSSProperties = { ...faceStyle, transform: 'rotateY(180deg) translateZ(0)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: 16 };

    return (
      <div style={{ width: '100%', height: '100%' }}>
        <div style={innerStyle}>
          <div style={faceStyle} className="w-full h-full">
            <img className="carousel-image gpu-accelerated w-full h-full object-cover" src={car.image} alt={`${car.brand} ${car.model}`} loading="lazy" style={{ filter: theme === 'dark' ? 'contrast(1.05) brightness(0.98) saturate(0.95)' : 'contrast(1.08) brightness(1.08) saturate(1.08)', transform: 'translateZ(0)', willChange: 'transform' }} />
          </div>
  ...
  return (
    <section className="relative w-full overflow-hidden py-8">
      <div className="relative z-10 max-w-6xl mx-auto px-4">
        ...
        <div
          ref={trackRef}
          className="carousel-track gpu-accelerated relative z-10 flex gap-6 overflow-x-auto no-scrollbar py-6 px-2 touch-pan-x"
          style={{ WebkitOverflowScrolling: 'touch', transform: 'translateZ(0)', willChange: 'transform, scroll-left' }}
          role="region"
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  ...
                  <div className="mt-4">
                    <div className="rounded-xl p-3 flex items-center justify-between" style={ theme === 'dark' ? { backdropFilter: 'blur(10px)', background: 'rgba(255,255,255,0.12)' } : { background: 'rgba(255,255,255,0.98)', boxShadow: '0 10px 30px rgba(16,24,40,0.08)', border: '1px solid rgba(0,0,0,0.06)' } }>
                      ...
                      <div>
                        {
                          // Prefetch page and api when user hovers/focuses/enters viewport
                        }
                        <Link
                          to={`/vehicle/${car.id}`}
                          className="text-sm text-[var(--accent)] font-semibold uppercase tracking-wide"
