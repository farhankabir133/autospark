  ...
  try{
    ...
    const result = await page.evaluate((STARTING_TEXT) => {
      function findCandidate() {
        ...
        for (const el of els) {
          // skip script/style/link elements which may contain comments or config
          const tag = el.tagName && el.tagName.toUpperCase();
          if (tag === 'SCRIPT' || tag === 'STYLE' || tag === 'LINK' || tag === 'META') continue;
          try {
            // match known chat widget classes even if there's no text
            const cls = (el.className || '') + '';
            if (cls.includes('chat-float') || cls.includes('chat-bubble') || cls.includes('chat-widget')) return el;
